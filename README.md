...
## CI/CD Pipeline for script App with Docker and Helm
    * Prerequisites:
            ...
            AZ_HOST: Public IP of the Azure VM; AVM_SSH_KEY: SSH private key to authenticate with the VM.
...
## Accessed the Application
    * Exposed the service associated with the deployment:  kubectl patch svc k8s-deploy-service \
    -p '{"spec": {"ports": [{"port": 80, "targetPort": 4499, "nodePort": 30371}], "type": "NodePort"}}'
    * Enabled inbound rules in the NSG (Network Security Groups)

## TLS Setup

![alt text](TLS-established/ssl_established.png)
